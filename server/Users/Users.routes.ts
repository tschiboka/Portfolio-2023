...
// GET /api/user — list all users (admin-facing).
...
// POST /api/user/register — register a new user (sends a confirmation email).
...
// POST /api/user/login — log a user in, returning an auth token + session data.
...
// POST /api/user/confirm — confirm a registration token and create the user.
router.post('/confirm', async (req: PostConfirmReq, res: PostConfirmRes) => {
    const data = await UsersService.confirm(req.body)
    ApiResponder.ok(res, data)
})

// GET /api/user/session — the current session's user and settings.
