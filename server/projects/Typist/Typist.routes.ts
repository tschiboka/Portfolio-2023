...
router.post('/round', [], async (req: PostRoundReq, res: PostRoundRes) => {
    ...
    if (TypistUserSettings.practiceMode === 'error') {
    ...
    const cutoffIndex = Math.floor((TypistUserSettings.difficulty / 100) * sortedFrequencies.length)
    ...
    const filteredWords = hasTargetFilteringCriteria
        ? filteredByDifficulty.filter((word: string) => {
              ...
              //   const meetsLetterCriteria = TypistUserSettings.targetLetter
              //       ? word.includes(TypistUserSettings.targetLetter)
              //       : true
    ...
    let responseWords = Arrays.times(
        TypistUserSettings.textLength,
        () => Arrays.random(wordPool) ?? '',
    )

    // Capitalisation
    if (!TypistUserSettings.allowCapitalLetters) {
    ...
    const calculateScore = () => {
        ...
        const accuracyScore = accuracy / 100 // 0â€“1
        ...
        // Factor 3: Derived from settings: words Ã— (avg word length + 1 for space separator)
    ...
    return ApiResponder.ok(res, {
        text: responseWords.join(' '),
        stats: {
            practiceMode: TypistUserSettings.practiceMode,
