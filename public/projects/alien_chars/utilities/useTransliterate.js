...
async function fetchCharacterMaps() {
    ...
    fetch(url)
      ...
      .then(data => { characterMaps = data; console.log("HERE", characterMaps) })
...
function displayCharacter(char) {
    ...
    const characterMap = characterMaps.find(characterMap => characterMap.characterName === char.combinationName.toLowerCase())
    if (characterMap) drawCharacter(svg, characterMap, char);
}

function drawCharacter(svg, map, characterProps) {
    if (!map) return;
    const characterMap = {characterName: map.characterName, characterLines: [...map.characterLines]}
    ...
    characterMap.characterLines.forEach(characterLine => {
        ...
        const [Y1, Y2] = [characterLine[1], characterLine[3]].map(coord => (coord + 1) * (height / 6))
...
function analyzeWordForDisplay(word, combinations, matches = []) {
    ...
    for (combination of combinations) {
        for(variations of combination) {
            ...
            if (matchCombinationResult.isMatch) {
                ...
                const partialMatchResult = analyzeWordForDisplay(matchCombinationResult.wordCharactersLeft, combinations, matches);
    ...
    if (!matched) {
        ...
        if (specialCharacterMatchResult?.isMatch) {
            ...
            else analyzeWordForDisplay(specialCharacterMatchResult.wordCharactersLeft, combinations, matches);
        }
    }
    return { word, matches }
}
    ...
    for(let variation of variations.variation) {
        ...
        if (matchingText === variation) {
            const individualVowel = individualChars.vowels.find(ch => {
                if (new RegExp(ch.short_version, "gi").test(matchingText)) vowelName = ch.short_version;
                if (new RegExp(ch.long_version, "gi").test(matchingText)) {
                    emphasisVowel = true;
                    vowelName = ch.short_version;
                }
                if (new RegExp("^" + ch.short_version, "gi").test(matchingText)) reverseCombination = true;
                if (new RegExp("^" + ch.long_version, "gi").test(matchingText)) reverseCombination = true;
            ...
            else { combinationName = variations.name; }
...
function matchSpecialCharacters(word) {
    ...
    if (word[0] === specialCharacter.character) return ({
