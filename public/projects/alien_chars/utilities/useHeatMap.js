    ...
    for (let rowIndex = 0; rowIndex <= consonants.length; rowIndex++) {
        ...
        for (let colIndex = 0; colIndex <= vowels.length + 1; colIndex++) {
            ...
            if (rowIndex === 0) {
                if (colIndex >= 2) {
                    const vowel = vowels[colIndex - 2]
                    const vowelMatchesShort = textAnalysis.filter(match => match.name.toLowerCase() === vowel.short_version)
                    const vowelMatchesLong = textAnalysis.filter(match => match.name.toLowerCase() === vowel.long_version)
                    const vowelMatchesShortCount = vowelMatchesShort?.length ? vowelMatchesShort[0].count : 0
                    const vowelMatchesLongCount = vowelMatchesLong?.length ? vowelMatchesLong[0].count : 0;
                ...
                if(colIndex === 0) {
                    ...
                    const consonant = consonants[rowIndex - 1]
                    const consonantMatchesShort = textAnalysis.filter(match => match.name.toLowerCase() === consonant.short_version)
                    const consonantMatchesLong = textAnalysis.filter(match => match.name.toLowerCase() === consonant.long_version)
                    const consonantMatchesShortCount = consonantMatchesShort?.length ? consonantMatchesShort[0].count : 0
                    const consonantMatchesLongCount = consonantMatchesLong?.length ? consonantMatchesLong[0].count : 0;
                    ...
                    const combinationMatches = textAnalysis.filter(match => match.name.toLowerCase() === combination)
