...
function preloadPics() {
    ...
    path = [
        ...
        'shards/apple_shard4',
        'shards/apple_shard5',
        'shards/blood_orange_shard1',
        'shards/blood_orange_shard2',
        'shards/blood_orange_shard3',
        'shards/blood_orange_shard4',
        'shards/blood_orange_shard5',
        'shards/kiwi_shard1',
        'shards/kiwi_shard2',
        'shards/kiwi_shard3',
        'shards/kiwi_shard4',
        ...
        'shards/lime_shard4',
        'shards/lime_shard5',
        'shards/orange_shard1',
        'shards/orange_shard2',
        'shards/orange_shard3',
        'shards/orange_shard4',
        'shards/orange_shard5',
        'shards/peach_shard1',
        'shards/peach_shard2',
        'shards/peach_shard3',
        'shards/peach_shard4',
        ...
        'shards/plum_shard4',
        'shards/plum_shard5',
        'shards/strawberry_shard1',
        'shards/strawberry_shard2',
        'shards/strawberry_shard3',
        'shards/strawberry_shard4',
        'shards/strawberry_shard5',
        'shards/blood_orange_shard1',
        'shards/blood_orange_shard2',
        'shards/blood_orange_shard3',
        'shards/blood_orange_shard4',
    ...
    try {
        path.forEach((p) => {
            ...
            img.src = `https://tschiboka.com/projects/fruits_n_flowers/images/${p}.png`
...
function preloadRest() {
    const path = [
        'walkthrough/bonus-I4H',
        'walkthrough/bonus-I4V',
        'walkthrough/bonus-I5CR',
        'walkthrough/bonus-I5X',
        'walkthrough/bonus-L51',
        'walkthrough/bonus-L52',
        'walkthrough/bonus-T5',
        'walkthrough/bonus-T6',
        'walkthrough/destroy-stones',
        'walkthrough/diamondcreation',
        'walkthrough/double-explosion',
        'walkthrough/flower-falls',
        'walkthrough/flower-horizontal-move',
        'walkthrough/inventory',
        'walkthrough/match-three',
        'walkthrough/match-I3',
        'walkthrough/match-I4H',
        'walkthrough/match-I4V',
        'walkthrough/match-I5',
        'walkthrough/match-L5',
        'walkthrough/match-O4',
        'walkthrough/match-T5',
        'walkthrough/match-T6',
        'walkthrough/match-T7',
        'walkthrough/menu',
        'walkthrough/shop',
        'walkthrough/swipe-flower-down',
        'walkthrough/three-flowers',
    ]
    ...
    try {
        path.forEach((p) => {
            ...
            img.src = `https://tschiboka.com/projects/fruits_n_flowers_v2/images/${p}.gif`
            app.images[picVarName] = img
        }) // end of forEach path
        ...
        photo.src = 'https://tschiboka.com/projects/fruits_n_flowers_v2/images/misc/photo.jpg'
        ...
        if (ind < levels.length) {
            ...
            if (app.game_max_points[ind]) {
                ...
                switch (true) {
                    ...
                    case percent >= 1 && percent < 25: {
                        className = 'under-twentyfive-percent'
                        break
                    }
                    case percent >= 25 && percent < 50: {
                        className = 'under-fifty-percent'
                        break
                    }
                    case percent >= 50 && percent < 75: {
                        className = 'under-seventyfive-percent'
                        break
                    }
                    case percent >= 75 && percent < 100: {
                        className = 'under-hundred-percent'
                        break
                    }
                    case percent >= 100: {
                        className = 'maxed'
            ...
            // ADD PADLOCKS
            const lockedFrom = !app.game_max_points[app.game_max_points.length - 1]
...
function addLevelEvents() {
    ...
    $('.level-menu__arrow-left').on('click', () => {
        turnLevelPage('-')
    })
    $('.level-menu__arrow-right').on('click', () => {
        turnLevelPage('+')
    })
    ...
    $('.level-menu__header__level-indicator').on('click', function (event) {
        ...
        if (/to-level-page-/g.test($(event.target).attr('class'))) {
            const pageNum = $(event.target)
                .attr('class')
                .match(/to-level-page-\d+/g)[0]
    ...
    $('.level-page').on('click', function (e) {
        const // get level number
            levelNum = [...e.target.classList] // extract array-like dom tokens to array
                ...
                .match(/\d+/g)
                .map(Number)[0] // extract the number
        ...
        if (levelNum && levels[levelNum - 1]) {
            ...
            if (!isLocked) {
                const // validate level
                    ...
                    props = [
                        'blueprint',
                        'fruitVariationNumber',
                        'minimumFlowersOnBoard',
                        'flowersToCompleteTheLevel',
                        'targetPoints',
                        'time',
                    ],
                    HASALLPROPS = props.every((pr) => L.hasOwnProperty(pr)), // check if level has any missing property
                    blueprint = L.blueprint || [[]],
                    blueprintIsCorrectSize =
                        blueprint.length === 11 && blueprint.every((row) => row.length === 9), // blueprint is 11 X 9 2D array
                ...
                if (VALIDLEVEL) {
                    ...
                    errorMsg += BLUEPRINTOK
                        ? ''
                        : '\tBLUEPRINT PROPERTY MISSING OR INVALID SIZE OR HAS INVALID VALUE\n'
                    errorMsg += FRUITOK
                        ? ''
                        : '\tFRUIT VARIATION NUMBER IS MISSING OR OUT OF RANGE (5 - 9)\n'
                    errorMsg += MINFLOWEROK
                        ? ''
                        : '\tMINIMUM FLOWERS ON LEVEL IS MISSING OR OUT OF RANGE (1 - 9)\n'
                    errorMsg += FLOWERSTOCOMPLETEOK
                        ? ''
                        : '\tFLOWERS TO COMPLETE LEVEL IS MISSING OR OUT OF RANGE (1 - 20)\n'
                    errorMsg += TARGETOK
                        ? ''
                        : '\tTARGET POINT IS MISSING OR OUT OF RANGE (500 - 10000)\n'
...
function styleLevelMenuToCurrenPage() {
    ...
    $('.level-menu__footer__text-container').html(
        `-&lt;[&nbsp;&nbsp;${activePageNum} / ${Math.ceil(levels.length / 25)}&nbsp;&nbsp;]&gt;-`,
    )
    ...
    $('.menu__open-close-btn-box')
        .on('click', () => {
            ...
            if (!$('.menu').hasClass('menu-open')) {
                ...
                $('.menu__open-close-arrow').addClass('arrow-open').removeClass('arrow-close')
...
function menuFunctions(action) {
    ...
    function giveMessage(text, callBackYes, callBackNo, buttonYesTxt, buttonNoTxt) {
        ...
        if (app.game_is_on) {
            ...
            $(buttonDiv).addClass('main-menu-msg-button-container').append(yesBtn).append(noBtn)

            // add inner div
            $(msgInnerDiv).addClass('main-menu-msg-inner').html(text).append(buttonDiv)

            // append the whole lot
            $(msgDiv).addClass('main-menu-msg').append(msgInnerDiv)
    ...
    function coldCloseLevel() {
        ...
        // reset level-points
        const lvlPts = $('.game-board__level-points').html().replace(/\d+/, '0')

        $('.game-board__level-points').html(lvlPts)

        $('header').removeClass('header--hidden header-out').addClass('header--visible header-in')
        $('.level-menu').show()
        setLevelMax()
    ...
    switch (action) {
        case 'back': {
            const txt =
                    'Are you sure you want to quit the current game and return to the level menu?',
                callBackYes = () => {
                    closeSelf()
                    coldCloseLevel()
                },
        ...
        case 'pause': {
            const txt = 'Game is paused! Press OK if you want to return to the game!',
                callBackYes = () => {
                    app.game_is_paused = false
                    closeSelf()
                }
...
function addGameBoardEvents() {
    ...
    $('.game-board__table').on('mousedown touchstart', function (event) {
        ...
        if (app.game_interaction_enabled) {
            ...
            const hintClasses =
                '.hint-horizontal-left, .hint-horizontal-right, .hint-vertical-upper, .hint-vertical-lower'
    ...
    function swapBonusClasses(r1, c1, r2, c2) {
        ...
        function swapElementsAndClasses(e1, e2) {
            ...
            $(elem1)
                ...
                .empty()
                .append($(elem2).children())

            $(elem2).removeClass('bonus').addClass(tempElemClass).empty().append(tempElemClone)
...
function addInventoryEvents() {
    ...
    function inventoryItemStart(event, dragObj = {}) {
        ...
        if (app.game_interaction_enabled) {
            if (
                $(target).hasClass('inventory-item-container') ||
                $(target).hasClass('inventory-item')
            ) {
    ...
    function dragInventoryItem(event, dragObj) {
        ...
        function createInventoryItemClone() {
            ...
            if (diffY > diffX && diffX < 40 && diffY > 15) {
                ...
                $(`.inventory-item:nth-child(${dragObj.itemNum})`).css('opacity', '0.2')
        ...
        function dragInventoryItemClone() {
            ...
            const // identify cells under the clone object
                underId = (
                    document.elementFromPoint(x + dragObj.width / 2, y + dragObj.height / 2) || {
                        id: '',
                    }
                ).id, // avoid null obj
            ...
            if (isUnderGameCell) {
                ...
                function createCrossightBorder(row, col) {
                    const // get neightbouring cells
                        ...
                        [N1, N2, N3] = Array(3)
                            .fill()
                            .map((_, i) => ({ el: getId(row - 1 - i, col), dist: i })), // NORTH ELEMENTS
                        [S1, S2, S3] = Array(3)
                            .fill()
                            .map((_, i) => ({ el: getId(row + 1 + i, col), dist: i })), // SOUTH ELEMENTS
                        [W1, W2, W3] = Array(3)
                            .fill()
                            .map((_, i) => ({ el: getId(row, col - 1 - i), dist: i })), // WEST ELEMENTS
                        [E1, E2, E3] = Array(3)
                            .fill()
                            .map((_, i) => ({ el: getId(row, col + 1 + i), dist: i })), // EAST ELEMENTS
                        opacity = [0.9, 0.6, 0.3]
                    ...
                    ;[N1, N2, N3, S1, S2, S3].forEach((el) => {
                        if (el.el) {
                            ...
                            $(el.el)
                                .css(
                                    'border-left',
                                    `1px solid rgb(200, 200, 200, ${opacity[el.dist]})`,
                                )
                                .css(
                                    'border-right',
                                    `1px solid rgb(200, 200, 200, ${opacity[el.dist]})`,
                ...
                const // check if cell under item is droppable aka is a fruit without existing bonus
                    [cellUnderRow, cellUnderCell] = underId
                        .match(/\d+/g)
                        .map(Number)
                        .map((e) => Math.abs(e)), // get row, cell
                    ...
                    fillRange = (
                        ...
                        Array(end - start)
                            .fill()
                            .map((_, i) => Math.abs(start + i)), // eg: fillRange(2,6) => [2, 3, 4, 5] excluding end
                    [toRow, fromRow] = [
                        fillRange(0, cellUnderRow).reverse(),
                        fillRange(cellUnderRow + 1, 11),
                    ], // from is ascending, to is decending
                    [toCol, fromCol] = [
                        fillRange(0, cellUnderCell).reverse(),
                        fillRange(cellUnderCell + 1, 9),
                    ], // eg: to [543210] 6 [789]
                    RED = [252, 69, 100], // rgb
                    GREEN = [88, 232, 243],
                    color = dropable ? GREEN : RED, // decide color
                    sequence = (length, start = 0.35, end = 0.075) =>
                        Array(length)
                ...
                toRow.forEach((rowInd, i) => {
                    createCrossightCell(
                        rowInd,
                        cellUnderCell,
                        `rgba(${color.join(',')},${sequence(toRow.length)[i]})`,
                    )
                ...
                fromRow.forEach((rowInd, i) => {
                    createCrossightCell(
                        rowInd,
                        cellUnderCell,
                        `rgba(${color.join(',')},${sequence(fromRow.length)[i]})`,
                    )
                ...
                toCol.forEach((colInd, i) => {
                    createCrossightCell(
                        cellUnderRow,
                        colInd,
                        `rgba(${color.join(',')},${sequence(toCol.length)[i]})`,
                    )
                ...
                fromCol.forEach((colInd, i) => {
                    createCrossightCell(
                        cellUnderRow,
                        colInd,
                        `rgba(${color.join(',')},${sequence(fromCol.length)[i]})`,
                    )
    ...
    function cancelInventoryItemDrag() {
        ...
        $(`.inventory-item:nth-child(${dragObj.itemNum})`).css('opacity', '1')
    ...
    $('.inventory-items').on('mousedown touchstart', function (e) {
        isMouseDown = true
        swipeStartCoord = swipeStart(e)
    })
    $('.inventory-items').on('mouseup touchend', function () {
        isMouseDown = false
        swipeStartCoord = 0
    })
...
function addShopEvents() {
    function shopSetup() {
        const fruitPics = [
                ...
                app.images.orange,
                app.images.peach,
                app.images.strawberry,
                app.images.plum,
                app.images.lime,
                app.images.kiwi,
                app.images.blood_orange,
                app.images.lemon,
            ], // end of fruitPics
        ...
        // add bonus pic
        $('.shop__bonuses > div').each(function (i) {
            createspecialGemDiv(null, bonusType[i], this)
        })
    ...
    function updateShop() {
        ...
        // display fruits
        fruitPics = {
            apple: app.images.apple,
            orange: app.images.orange,
            peach: app.images.peach,
            strawberry: app.images.strawberry,
            plum: app.images.plum,
            lime: app.images.lime,
            blood_orange: app.images.blood_orange,
            kiwi: app.images.kiwi,
            lemon: app.images.lemon,
        } // end of fruitPics

        if (app.shop_basket.fruit) {
            $('.shop__display').css(
                'background-image',
                `url(${fruitPics[app.shop_basket.fruit].src})`,
            )
    ...
    function buyContentOfBasket() {
        ...
        if (app.game_total_points >= app.shop_basket.price) {
            ...
            $(msgDiv).addClass('insufficient-funds-msg').html('Insufficient funds!').append(okBtn)
    ...
    $('#shop-icon').on('click', () => {
        if (app.game_interaction_enabled) {
            ...
            app.game_best_hint
                ? $('#shop__best-hint').css('color', 'rgb(43, 34, 49)')
                : $('#shop__best-hint').css('color', '#58e8f3')

            app.game_give_hint_at !== 10
                ? $('#shop__fast-hint').css('color', 'rgb(43, 34, 49)')
                : $('#shop__fast-hint').css('color', '#58e8f3')
            ...
            if (match.sample !== 'X') {
                ...
                for (i = 0; i < match.coords.length; i++) {
                    ...
                    for (sh = 0; sh < 5; sh++) {
                        ...
                        $(shard).css({
                            top: idXY.y - parentXY.y + diffY[sh],
                            left: idXY.x - parentXY.x + diffX[sh],
                        })
...
function fillBoardWithNewFruits() {
    ...
    function fillEmptyCell(num) {
        const randFruit =
            Math.ceil(Math.random() * levels[app.currentLevel - 1].fruitVariationNumber) + ''
        app.board[idsToFill[num][0]][idsToFill[num][1]] = randFruit

        // check if there is enough flowers on the board
        const currentFlowers = [...app.board.join('').replace(/,/g, '')].filter((el) =>
            /[ABCDE]/g.test(el),
        ).length
        ...
        if (currentFlowers < levels[app.currentLevel - 1].minimumFlowersOnBoard) {
            ...
            const topmostRow = idsToFill[idsToFill.length - 1][0][0],
                ...
                cellIds = idsToFill.filter((id) => id[0][0] === topmostRow).map((id) => id[1][0]),
                // choose one randomly
...
function displayLevelPoints() {
    ...
    if (app.game_partial_points < 6) {
        ...
        const increasePointsDelay = setInterval(() => {
            counter--
            let point = $('.game-board__level-points').html().match(/\d+/g)[0] // extract points
        ...
        let counter = 0,
            point = $('.game-board__level-points').html().match(/\d+/g)[0] // extract points

        const points = [...Array(app.game_partial_points).keys()].map(
            (p) => Number(p) + Number(point),
        )
        let randomPoints = []
    ...
    app.board.forEach(
        (row, rowPos) =>
            row.forEach((cell, cellPos) => {
                if (cell === 'U' && isFlower((app.board[rowPos - 1] || [])[cellPos]))
                    flowerOverBasket++
            }), // end of cell
    ) // end of row
    ...
    basketsPos.forEach((basket) => {
        ...
        if (basket[0] !== 0) {
            if (isFlower(basket[0] - 1, basket[1])) {
                ...
                let flowersLeftToCompleteLevelReq =
                    levels[app.currentLevel - 1].flowersToCompleteTheLevel - app.flowers
                $('#flower-counter').html(
                    flowersLeftToCompleteLevelReq > 0 ? flowersLeftToCompleteLevelReq : 0,
                )
...
function getSpecialGems(matches) {
    ...
    matches.forEach((match) => {
        ...
        if (
            match.patternName !== 'I3' &&
            match.patternName !== 'T7' &&
            match.patternName !== 'O4'
        ) {
        ...
        if (match.patternName === 'O4') {
            ...
            $(fiveSec).attr('id', 'five-sec').html('+5 sec')
            ...
            let coordsMatch = $(
                `#r${match.coords[0][0]}c${match.coords[0][1]}-box`,
...
function createspecialGemDiv(coord, name, elementSelector) {
    ...
    switch (name) {
        ...
        case 'I5X': {
            // in case I5X is given (only in inventory otherwise randomly given I5X / I5CR)
            addDivToBonusDiv('bonus-diagonal1-line')
            addDivToBonusDiv('bonus-diagonal2-line')
            $(specialDiv).addClass(`bonus-sign SIGN-I5X`)
            break
        ...
        case 'I5CR': {
            // in case I5CR is given (only in inventory otherwise randomly given I5X / I5CR)
            addDivToBonusDiv('bonus-vertical-line')
            addDivToBonusDiv('bonus-horizontal-line')
            $(specialDiv).addClass(`bonus-sign SIGN-I5CR`)
            break
    ...
    app.board.forEach((row, rowInd) => {
        row.forEach((cell, cellInd) => {
            ...
            if (cell === 'X' && elem.hasClass('bonus')) {
                ...
                const bonusType = elem.children().attr('class').split('SIGN-')[1]

                // remove bonus
                $(elem).removeClass('bonus').empty()
...
function bonusExplode(bonusType, rowInd, cellInd) {
    ...
    const exploseDiagonal = (dirR, dirC) =>
        [...Array(8).keys()].forEach((dist) => explose(rowInd + dist * dirR, cellInd + dist * dirC))
        ...
        coords.forEach((coord) => {
            ...
            cellsAroundHaveStone.forEach((cellAround, ind) => {
                const direction = [
                    ...
                    [0, -1],
                    [0, 1],
                ][ind] // returns the index the stone is on
...
function possibleMoves() {
            ...
            if (isMobileChar(cell)) {
                ...
                if (cellInd < 8 && isMobileChar(app.board[rowInd][cellInd - 1])) {
                    function checkRightSwipe() {
                        //direction functions are created so I can escape them separately
                        if (
                            checkPattern(
                                'T7',
                                [
    ...
    moves = moves.concat(combined).sort((movA, movB) => movB.coords.length - movA.coords.length)
    ...
    app.board.map((row, rowInd) =>
        row.map((cell, cellInd) => {
            ...
            if (availableFruits.some((fr) => fr == cell)) {
                ...
                if (
                    (app.board[rowInd - 1] || [])[cellInd] ===
                    (app.board[rowInd - 2] || [])[cellInd]
                ) {
                    // take them out from the pool of choices
                    availableFruits = availableFruits.filter(
                        (fr) => fr != (app.board[rowInd - 1] || [])[cellInd],
                    )
                }
                // if the two cells on the left are already matching
                if (
                    (app.board[rowInd] || [])[cellInd - 1] ===
                    (app.board[rowInd] || [])[cellInd - 2]
                ) {
                    // take them out from the pool of choices
                    availableFruits = availableFruits.filter(
                        (fr) => fr != (app.board[rowInd] || [])[cellInd - 1],
                    )
                }
                app.board[rowInd][cellInd] =
...
function closeLevel() {
    ...
    $(timeIsUp).html('TIME IS UP!').attr('id', 'time-is-up')
...
function showLevelStats() {
    ...
    if (app.flowers < levels[app.currentLevel - 1].flowersToCompleteTheLevel) {
        ...
        $(notCompletedDiv)
            ...
            .html(
                'Level has not been completed! On this level you need to collect ' +
                    levels[app.currentLevel - 1].flowersToCompleteTheLevel +
                    ' flower' +
                    pluralS +
                    ', and your basket has ' +
                    app.flowers +
        ...
        $('.game-board__level-points').html().replace(/\d+/, '0')
    ...
    // if the requrement has been completed show level stats and add points
        ...
        app.game_max_points[app.currentLevel - 1] =
            max <= app.game_level_points ? app.game_level_points : max
        ...
        const animPointsDelay = setInterval(() => {
            ...
            if (counter <= 0) {
                ...
                // update total points
                const totPts = $('.game-board__total-points').html().replace(/\d+/, tot) // the new total earned after a level completition
...
function rewardUser() {
    ...
    if (extraFlowers > 0) {
        ...
        switch (rest) {
            case 4: {
                rewardsArr.push(fruit + '-T6')
                break
            }
            case 3: {
                rewardsArr.push(fruit + '-' + randomArrayElement(['I5CR', 'I4V']))
                break
            }
            case 2: {
                rewardsArr.push(fruit + '-' + randomArrayElement(['I5X', 'T5']))
                break
            }
            case 1: {
                rewardsArr.push(fruit + '-' + randomArrayElement(['I4H', 'L51', 'L52']))
                break
            }
        } // end of swith rest
        ...
        const rewardMsgDiv = document.createElement('div'),
        ...
        $(rewardMsgDiv)
            ...
            .html(
                `<div>You collected ${extraFlowers} more flower${pluralS(extraFlowers)} than the level requirement.` +
...
function startLevel(level) {
    ...
    // arrange new layout
    $('.level-menu').hide()
    $('header').removeClass('header--visible header-in').addClass('header--hidden header-out')
    ...
    app.game_give_hint_at = 10
    app.game_current_level_maxed =
        app.game_max_points[app.currentLevel - 1] >= levels[app.currentLevel - 1].targetPoints
    ...
    const levelTarget = levels[app.currentLevel - 1].targetPoints,
        divTxt = $('.game-board__level-points')
            .html()
            .replace(/\/\d+/, '/' + levelTarget)

    $('.game-board__level-points').html(divTxt)

    // reset level-points
    $('.game-board__level-points').html($('.game-board__level-points').html().replace(/\d+/, '0'))
...
function displayInventoryItems() {
    ...
    $('.game-board__inventory__menu__indicator').children().removeClass('active')
...
function createBoardArray(level) {
    ...
    if (board.length !== 11) {
        return Error(
            'The length of game-board is incorrect, it must be 11, instead it is :' +
                board.length +
                '!',
        )
    ...
    board.map((row, rowInd) => {
        if (row.length !== 9) {
            return Error(
                `Invalid length of row[${rowInd}]. It must be 9, instead it is ${row.length}!`,
            )
...
function displayBoard() {
    ...
    for (r = 0; r < 11; r++) {
        for (c = 0; c < 9; c++) {
            ...
            if (app.board[r][c] === '.') {
                let availableFruits = [
                    ...Array(levels[app.currentLevel - 1].fruitVariationNumber).keys(),
                ].map((el) => ++el)
...
function setAppValuesFromLocalStorage() {
    ...
    if (
        storage.getItem('game_total_points') === null ||
...
function offerWalkthrough() {
    ...
    $(offerMsgTextDiv)
        ...
        .html(
            'Hi, it looks like this is your first time visiting this page!<br /><br />Let me offer you a quick walkthrough of the game-play in case you are not familiar with it.',
        )
    ...
    $(offerMsgDiv).addClass('offer-msg').append(offerMsgInnerDiv)
...
function openWalkthrough() {
    ...
    const contentHeight = $('.walkthrough__content')[0].scrollHeight,
        scrollbarHeight = Math.round(
            $('.walkthrough__scrollbar')[0].getBoundingClientRect().height,
        ),
...
function createWalkthroughAndAboutEvents() {
    ...
    $('.walkthrough').on('mousemove touchmove', (event) => {
        ...
        if (scrollBtnDown) {
            const newCursorY = event.pageY || event.changedTouches[0].pageY,
            ...
            if (newY > 0 && newY <= scrollBarHeight - scrollBtnHeight) {
                ...
                const newThumbPos = Math.floor(
                    ((scrollBarHeight - scrollThumbHeight) / 100) * percent,
                )

                $('#scrollbar-thumb').css('bottom', newThumbPos + 'px')

                // scroll content
                const newContentPos = Math.floor(
                    ((contentScrollHeight - contentHeight) / 100) * percent,
                )
    ...
    function setMatchTable() {
        ...
        const matchNames = [`I3`, `I4H`, `I4V`, `O4`, `T5`, `L5`, `I5`, `T6`, `T7`],
            dir = 'https://tschiboka.com/projects/fruits_n_flowers_v2/images/walkthrough/'

        $('.table-picture').css(
            'background-image',
            `url("${dir}matches-${matchNames[app.match_table_ind - 1]}.gif")`,
        )
        ...
        switch (app.match_table_ind) {
            ...
            case 9: {
                bonusContent =
                    "Diamond <img src='https://tschiboka.com/projects/fruits_n_flowers_v2/images/misc/diamond.png' alt='bonus' />"
            } // end of case T7
