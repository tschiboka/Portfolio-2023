...
function gameLoop() {
    ...
    if (app.play) {
            ...
            if (start == app.noteIndex) {
                ...
                noteStr.split(',').forEach((n) => {
                    ...
                    const noteTimer = setTimeout(() => {
                        // Create a Timer Function to Stpo the Note
        ...
        if (app.noteIndex === MAXBEAT) {
            ...
            if (app.barIndex >= to) if (app.repeatAllowed) app.barIndex = from - 1 // Reset Bar Index
            app.noteIndex = 0 // Reset Note Index

            if (app.barIndex >= app.tab.bars.length) {
                // If No More Bars to Play
                if (app.isRecordAllowed)
                    app.tab.bars.push([]) // Always Append Tab While Recording
...
function setTempo() {
    ...
    const content =
        "The music sheet's tempo may be set for a different tempo. The tempo measurement is in BPM (Beat per Minute), and its value must be between 20 and 300."
    ...
    const input = $append({
        ...
        className: 'input',
        parent: messageBox,
    }) // Append Message Action Button Box Div
...
function editBeat(event) {
    ...
    const chordNameInput = $append({
        tag: 'input',
        id: 'chord-name-input',
        parent: chordNameInputField,
    })
    ...
    durs.forEach((d) => {
        ...
        const url =
            'url(../../images/music-notations/' + d.toLocaleLowerCase().replace(/_/g, '-') + '.png)'
...
function validateNote(values) {
    if (values.chordName && values.chordName.length > 10)
        return { valid: false, message: 'Chord Name Must is Max 10 Character' }
    if (values.E && isNaN(values.E)) return { valid: false, message: 'String E Must Be a Number' }
    if (values.E && Number(values.E < 0))
        return { valid: false, message: 'String E Must Be Positive' }
    if (values.E && Number(values.E > 20))
        return { valid: false, message: 'String E Must Be Less Than 20' }
    if (values.A && isNaN(values.A)) return { valid: false, message: 'String A Must Be a Number' }
    if (values.A && Number(values.A < 0))
        return { valid: false, message: 'String A Must Be Positive' }
    if (values.A && Number(values.A > 20))
        return { valid: false, message: 'String A Must Be Less Than 20' }
    if (values.D && isNaN(values.D)) return { valid: false, message: 'String D Must Be a Number' }
    if (values.D && Number(values.D < 0))
        return { valid: false, message: 'String D Must Be Positive' }
    if (values.D && Number(values.D > 20))
        return { valid: false, message: 'String D Must Be Less Than 20' }
    if (values.G && isNaN(values.G)) return { valid: false, message: 'String G Must Be a Number' }
    if (values.G && Number(values.G < 0))
        return { valid: false, message: 'String G Must Be Positive' }
    if (values.G && Number(values.G > 20))
        return { valid: false, message: 'String G Must Be Less Than 20' }
    if (values.B && isNaN(values.B)) return { valid: false, message: 'String B Must Be a Number' }
    if (values.B && Number(values.B < 0))
        return { valid: false, message: 'String B Must Be Positive' }
    if (values.B && Number(values.B > 20))
        return { valid: false, message: 'String B Must Be Less Than 20' }
    if (values.e && isNaN(values.e)) return { valid: false, message: 'String e Must Be a Number' }
    if (values.e && Number(values.e < 0))
        return { valid: false, message: 'String e Must Be Positive' }
    if (values.e && Number(values.e > 20))
    ...
    if (values.duration) {
        if (!(values.E || values.A || values.D || values.G || values.B || values.e))
            return { valid: false, message: 'Minimum 1 String Must be Set' }
    }
    if (values.chordName) {
        if (
            !(
            ...
            return { valid: false, message: 'Cannot Set Only Chord Name!' }
    }
...
function moveNote(count, beatInfo, times = 0) {
    ...
    if (noteIndex === undefined)
        noteIndex = Number(beatInfo.target.id.match(/beat\d+/g)[0].replace('beat', ''))
...
async function openTabForm() {
    ...
    if (success) {
        ...
        tabs.forEach((tab, i) => {
            ...
            publicElem.innerHTML = tab.isPublic
...
async function deleteTab() {
    ...
    if (success) {
        ...
        if (savedTab) {
            ...
            const content =
                'You have a saved tab with the title: ' +
                app.tab.title +
                '. Would you like to permanently delete it from our database?'
...
function handleStrumActivated(event, strum) {
    ...
    if (event === 1)
        finaliseNote(strum) // Strum Pressed
    else {
        // Strum Released
...
function handleFretActivated(string) {
    // Changes on the Strings Fret Would Finalise the Note
    if (!app.isRecordAllowed) return // Return If Not Recording
    finaliseNote(string)
}
...
function placeUnfinalisedNoteOnTab(barIndex, noteIndex, upperMostFret, strum) {
    ...
    if (existNoteIndex) {
        ...
        const removeTimeout = setTimeout(
            () => removeWithLongestDuration(unfinalised, removeTimeout),
            removeTime,
        )
    }
