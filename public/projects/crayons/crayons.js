...
;((ctx = canvas.getContext('2d')), // canvas context
    ...
    (toolSettings = {
        // the collection of the tools attributes
...
function addPencilMouseListeners() {
    ...
    colors.map((color) => {
        ...
        pseudo.addEventListener('click', () => {
            ...
            document.getElementById('color-selector-dialog-box').value =
                htmlColorToHex(drawingColor)
        }) // end of click
...
function addCursorOverCanvasListener() {
    canvas.addEventListener('mousemove', function (e) {
        ...
        canvasX = X
        canvasY = Y // pass values to global vars
        if (mouseDown) {
            switch (tool) {
                case 'draw': {
                    drawOnCanvas()
...
function addMouseUpDownListener() {
    ...
    body.addEventListener('mouseup', () => {
        mouseDown = false
        lastDrawEventCoordinates = false
    })
...
function drawOnCanvas() {
    ...
    if (!code.length) {
        ...
        allPrevColors = code
            .map(
                (
                    block, // run through blocks
                ) => (block.match(/fillStyle = (.*?);/g) || [''])[0],
            ) // if its strokestyle match it, if theres not return ""
...
function addIconListeners() {
    ...
    pencilIcon.addEventListener('click', () => {
        closeAllPanels()
        if (!disableIcons)
            checkManager('clear', 'set', document.getElementById('pencil-icon-check'))
    ...
    gridIcon.addEventListener('click', () => {
        if (
            window.getComputedStyle(document.getElementById('grid-canvas')).visibility === 'hidden'
        ) {
...
function addPointWidthSliderListener() {
    ...
    slider.addEventListener('change', () => {
        setWidth()
    }) // I had to write the literal clicks too (slider body didnt react)
    ...
    slider.addEventListener('mousedown', () => {
        sliderClicked = true
        setWidth()
    }) // if clicked change values
    slider.addEventListener('mousemove', () => {
        // if dragged change value as well
        if (sliderClicked) {
            document.getElementById('point-width-display').innerHTML = toolSettings.drawingWidth =
                slider.value
        } // end of if
...
function addPositioner(worktop, num, coords) {
    ...
    if (num !== coords.length)
        throw new Error(
            "Error on calling addPositioner function! Coordinates length don't match! (" +
                num +
                ') (' +
                coords.length +
                ')',
...
function anyShapeDrawing(positionerNum, drawingFunction) {
    ...
    let oldPositionX,
        oldPositionY, // they're gonna get there value from mousedown event
        helper,
        helperYes,
        helperNo,
        helperMin,
        helperMax,
        helperRad,
        helperNum, // their value will depend on tools setting
    ...
    switch (tool) {
        case 'rounded-rectangle': {
            ...
            ;((helperMin = document.getElementById('worktop-helper-rounded-rectangle-arrow-left')), // left arrow decrease number
                (helperMax = document.getElementById(
                    'worktop-helper-rounded-rectangle-arrow-right',
    ...
    helperNo.addEventListener('click', () => {
        disableIcons = false
        closeWorkTop()
    })
    ...
    ;[...positioners].forEach((e) => {
        ...
        e.addEventListener('contextmenu', function (event) {
            ...
            if (tool === 'rounded-rectangle') {
                // can select and deselect corners
    ...
    workTop.addEventListener('mouseup', function () {
        positionerClicked.map((e, i) => (positionerClicked[i] = false))
    }) // workaround, positionerClicked is a constans, can't re-reference it

    workTop.addEventListener('mousemove', function (event) {
        const positionerToDrag = document.getElementById(
                'positioner' + Number(positionerClicked.findIndex((e) => !!e) + 1),
            ),
            leftmostX = Math.max(
                ...[...positioners].map((p) =>
                    Number(window.getComputedStyle(p).left.match(/\d+/)),
                ),
            ), // find leftmost x position
            leftMostPositionerY = window.getComputedStyle(
                positioners[
                    [...positioners].findIndex(
                        (p) => window.getComputedStyle(p).left === leftmostX + 'px',
                    )
                ],
            ).top
            ...
            switch (tool) {
                case 'square': {
                    const corner = Number(positionerToDrag.id.match(/\d/) - 1),
                        findPositionerWherXIsToBeAdjusted = (c) =>
                            [
                                [0, 1],
                                [1, 0],
                                [2, 3],
                        ...
                        findPositionerWherYIsToBeAdjusted = (c) =>
                            [
                                [0, 3],
                                [1, 2],
                                [2, 1],
                        ...
                        xAdjust = document.getElementById(
                            'positioner' + (findPositionerWherXIsToBeAdjusted(corner) + 1),
                        ),
                        yAdjust = document.getElementById(
                            'positioner' + (findPositionerWherYIsToBeAdjusted(corner) + 1),
                        )
                    ...
                    switch (corner) {
                        case 0: {
                            changeX < 0 && changeY < 0
                                ? (changeXY = [-Math.abs(changeAmount), -Math.abs(changeAmount)])
                                : changeX >= 0 && changeY >= 0
                                  ? (changeXY = [Math.abs(changeAmount), Math.abs(changeAmount)])
                                  : (changeXY = [0, 0])
                            break
                    ...
                    const X =
                            0 < left + changeXY[0] && left + changeXY[0] < 380
                                ? left + changeXY[0]
                                : left,
                        Y =
                            0 < top + changeXY[1] && top + changeXY[1] < 300
                                ? top + changeXY[1]
                                : top
                ...
                case 'rounded-rectangle': {
                } // FALLTRUOGH IS INTENTIONAL !!!!! rounded rect has the same positioner bounding as rect
                case 'rectangle': {
                    const corner = Number(positionerToDrag.id.match(/\d/) - 1),
                        findPositionerWherXIsToBeAdjusted = (c) =>
                            [
                    ...
                    positionerToDrag.style.left =
                        (newXY[0] >= -4 && newXY[0] <= 375 ? newXY[0] : left) + 'px' // positions have to be in worktop!
                    positionerToDrag.style.top =
                        (newXY[1] >= -4 && newXY[1] <= 295 ? newXY[1] : top) + 'px'
                    xAdjust.style.left = window.getComputedStyle(positionerToDrag).left
                    yAdjust.style.top = window.getComputedStyle(positionerToDrag).top
                    break
                } // end of case rectangle
                case 'arc': {
                    const coordsX = [...positioners].map(
                            (e) => +window.getComputedStyle(e).left.match(/\d+/g)[0],
                        ), // the positioners coordinates
                        coordsY = [...positioners].map(
                            (e) => +window.getComputedStyle(e).top.match(/\d+/g)[0],
                        ),
        ...
        const allPrevWidths = code.map((block) => block.match(/(lineWidth = )\d+/g)), // find all linewidths and extract them
            ...
            allPrevDashes = code
                .map(
                    (
                        block, // run through blocks
                    ) => (block.match(/setLineDash.+;/g) || [''])[0],
                ) // if its dash match it, if theres not return ""
                .filter((line) => line !== ''), // get rid of ""-s
...
function setRoundedRectangle() {
    ...
    function drawRoundedRectangle(context) {
        const X1 = positioners[0].style.left,
            ...
            Y4 = positioners[3].style.top,
            R = Number(
                document.getElementById('worktop-helper-rounded-rectangle-number').innerHTML,
            ), // radius
            W = (cond, w = toolSettings.drawingWidth / 2) => (cond ? w : -w), // the edges were a littlebit clumpsy, that's the correction
            SEL = [...positioners].map(
                (e) => window.getComputedStyle(e).backgroundColor === 'rgb(250, 115, 115)',
            ), // find selected positioners by background color
            // get rid of all px postfixes (+ 4 is to get it centered (positioners width n height is 8px with border))
            coords = [X1, Y1, X2, Y2, X3, Y3, X4, Y4].map((e) => Number(e.match(/\d+/)) + 4),
            [x1, y1, x2, y2, x3, y3, x4, y4] = [...coords] // spread back the numbers
        ...
        // CORNER 1
        if (!SEL[0]) {
            // draw arc if pos1 not selected
            R >= 0
                ? roundedRectangleCtx.arc(x1 + R, y1 + R, R, Math.PI, Math.PI * 1.5, false) // positive
        ...
        if (!SEL[1]) {
            // draw arc if pos2 not selected
            R >= 0
                ? roundedRectangleCtx.arc(x2 + R, y2 - R, R, Math.PI, Math.PI * 0.5, true) // positive
        ...
        if (!SEL[2]) {
            // draw arc if pos3 not selected
            R >= 0
                ? roundedRectangleCtx.arc(x3 - R, y3 - R, R, Math.PI * 0.5, 0, true) // positive
        ...
        if (!SEL[3]) {
            // draw arc if pos4 not selected
            R >= 0
                ? roundedRectangleCtx.arc(x4 - R, y4 + R, R, Math.PI * 0, Math.PI * 1.5, true) // positive
        ...
        chunkOfCode +=
            `ctx.moveTo(${x1}, ${y1 + (!SEL[0] ? Math.abs(R) : 0) - W(y1 < y2)});\n` +
            `ctx.lineTo(${x2}, ${y2 - (!SEL[1] ? Math.abs(R) : 0)});\n`
        if (!SEL[1]) {
            chunkOfCode +=
                R >= 0
                    ? `ctx.arc(${x2 + R}, ${y2 - R}, ${R}, Math.PI, Math.PI * 0.5, true);\n`
                    : `ctx.arc(${x2}, ${y2}, ${Math.abs(R)}, Math.PI * 1.5, 0, false);\n`
        } // corner 2 code
        chunkOfCode +=
            `ctx.moveTo(${x2 + (!SEL[1] ? Math.abs(R) : 0) - W(x2 < x3)}, ${y2});\n` +
            `ctx.lineTo(${x3 - (!SEL[2] ? Math.abs(R) : 0)}, ${y3});\n`
        if (!SEL[2]) {
            chunkOfCode +=
                R >= 0
                    ? `ctx.arc(${x3 - R}, ${y3 - R}, ${R}, Math.PI * 0.5, 0, true);\n`
                    : `ctx.arc(${x3}, ${y3}, ${Math.abs(R)}, Math.PI, Math.PI * 1.5 , false);\n`
        } // corner 3 code
        chunkOfCode +=
            `ctx.moveTo(${x3}, ${y3 - (!SEL[2] ? Math.abs(R) : 0) + W(y3 > y4)});\n` +
            `ctx.lineTo(${x4}, ${y4 + (!SEL[3] ? Math.abs(R) : 0)});\n`
        if (!SEL[3]) {
            chunkOfCode +=
                R >= 0
                    ? `ctx.arc(${x4 - R}, ${y4 + R}, ${R}, Math.PI * 0, Math.PI * 1.5, true);\n`
        ...
        toggle.addEventListener('click', () => {
            ...
            if (toggle.dataset.on === 'true') {
                ...
                if (otherToggle.dataset.on === 'true') {
                    otherToggle.style.webkitAnimation = otherToggle.style.animation =
                        '0.2s toggleOff linear'
...
function getDashValues() {
    const get = (el) => document.getElementById(el),
        tgs = [
            get('dash-and-gap-toggle-button').dataset.on,
            get('dash-pattern-toggle-button').dataset.on,
        ]

    let val = []

    if (tgs[0] === 'true') {
        val = [get('dash-slider').value, get('gap-slider').value].map(Number)
    }
...
function checkManager(...commands) {
    ...
    commands.forEach((command, i) => {
        switch (command) {
            case 'log': {
                // Noop, just for debugging purposes to log the state of checks
                break
            } // end of case log
            case 'clear': {
                clearAll()
            ...
            case 'set': {
                ...
                if (all.filter((e) => checkSign === e).length === 1) {
                    // if next argument is a check element
                    ...
                    throw Error(
                        'After set command the next argument needs to be a valid check-sign element!',
                    )
                } // end of else
                break
            }
            case '': {
                break
            } // empty command has no any effect and is a valid command
            default: {
                throw Error(
                    'checkManager function cannot recognise ' + command + ' as a valid command!',
                )
            }
        ...
        tool = [
            ...
            'circle',
            'ellipse',
        ][[pencil, ...subLines, ...subShapes].findIndex((e) => isOn(e))]
...
function addArrowIconsListener() {
    ...
    function moveToolIcons(num) {
        ...
        const sequence = Array(tools.length)
                .fill()
                .map((e, i) => i), // fill array like [0, 1, 2, 3]...
            ...
            toolsOnDisplay = Array(5)
                .fill()
                .map((e, i) => tools[onDisplay[i]]) // an aray of tool HTML elements
