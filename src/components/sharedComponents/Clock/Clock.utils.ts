...
const drawLineFromCentre = (
    ...
    const props = {
        start: {
            x: { value: 50, type: 'pc' as const },
            y: { value: 50, type: 'pc' as const },
        },
        end: {
            x: { value: x },
            y: { value: y },
...
const drawFromCentre = (
    ...
    const props = {
        start: {
            x: { value: centre.x, type: 'px' as const },
            y: { value: centre.y, type: 'px' as const },
        },
        radius,
...
export const drawGrooves = (canvasObj: CanvasType) => {
    ...
    degreesMin.map((degree) =>
        drawLineFromCentre(centre, getAngle(degree, 60), radius, 1, '#333', line),
    )

    const degreesHour = [...Array(12).keys()].map((d) => d * 5)
    degreesHour.map((degree) =>
        drawLineFromCentre(centre, getAngle(degree, 60), radius, 3, '#333', line),
    )
